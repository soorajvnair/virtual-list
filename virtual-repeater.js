export const Repeats = Superclass => class extends Superclass {
  ...
  set totalItems(len) {
    if (typeof len === 'number' && len !== this._totalItems) {
      this._totalItems = len;
      this.first = this._first;
      this.requestReset();
    }
  }
   ...
   */
  _shouldRender() {
    return Boolean(this.totalItems >= 0 && this.container && this.newChild);
  }
   ...
   */
  _assignChild(idx) {
    const key = this.itemKey ? this.itemKey(idx) : idx;
    let child;
